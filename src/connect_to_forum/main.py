...
def main(event, context):
    ...
    print('DBG.F.2.:Pub/sub:', message_in_ascii)
    # print(str(tg_user_id), type(tg_user_id))
    # print(f_username)
    ...
    if user_found:
        msg = 'Посмотрите, Бот нашел следующий аккаунт на форуме, это Вы?\n'
        msg += 'username: ' + f_username + ', '
        if user.callsign:
            msg += 'позывной: ' + user.callsign + ', '
        if user.region:
            msg += 'регион: ' + user.region + ', '
        if user.phone:
            msg += 'телефон оканчивается на ' + str(user.phone)[-5:] + ', '
        if user.age:
            msg += 'возраст: ' + str(user.age) + ', '
        if user.reg_date:
            msg += 'дата регистрации: ' + str(user.reg_date)[:-7] + ', '
        msg = msg[:-2]
        ...
        cur.execute("""DELETE FROM user_forum_attributes WHERE
        user_id=%s;""", (tg_user_id,))
        conn_psy.commit()

        # Add new record for this user
        cur.execute("""INSERT INTO user_forum_attributes 
        (user_id, forum_user_id, status, timestamp, forum_username, forum_age, forum_sex, forum_region, forum_auto_num, forum_callsign, forum_phone, forum_reg_date) 
        ...
        msg = 'Бот не смог найти такого пользователя на форуме. Пожалуйста, проверьте правильность написания имени пользвателя (логина). Важно, чтобы каждый знак в точности соответсовал тому, что указано в вашем профиле на форуме'
        keyboard = [['связать аккаунты бота и форума'], ['в начало']]

    reply_markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True)
    bot.sendMessage(chat_id=tg_user_id, text=msg, reply_markup=reply_markup, parse_mode='HTML')
