...
class DBClient:
    ...
    def save_last_api_call_time_to_psql(self, geocoder: str) -> bool:
        ...
        with self.connect() as conn:
            try:
                ...
                logging.exception(f'UNSUCCESSFUL saving last api call time to geocoder {geocoder}')
    ...
    def update_coordinates_in_db(self, search_id: int, coords: tuple[float, float, str]) -> None:
        with self.connect() as conn:
            stmt = sqlalchemy.text("""
                ...
                FROM search_coordinates WHERE search_id=:a LIMIT 1;
                                   """)
                ...
                if old_coords is None:
                    ...
                    if do_update:
                        stmt = sqlalchemy.text("""
                            ...
                            SET latitude=:a, longitude=:b, coord_type=:c, upd_time=CURRENT_TIMESTAMP WHERE search_id=:d; 
                                               """)
    ...
    def write_search(self, line: SearchSummary) -> None:
        stmt = sqlalchemy.text("""
