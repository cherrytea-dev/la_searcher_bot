...
class DBClient(DBClientBase, DBKeyValueStorageMixin):
        ...
        with self.connect() as conn:
            ...
            stmt = sqlalchemy.text("""
                ...
                WHERE search_id=:a AND address=:b;
                                   """)
            prev_data = conn.execute(stmt, dict(a=search_num, b=address_string)).fetchone()
            ...
            if not prev_data:
                stmt = sqlalchemy.text("""
                    ...
                    VALUES (:a, :b, :c); 
                                       """)
                conn.execute(stmt, dict(a=search_num, b=address_string, c=datetime.now()))
        ...
        with self.connect() as conn:
            stmt = sqlalchemy.text("""
                ...
                ORDER BY id DESC LIMIT 1; 
                                   """)
            saved_result = conn.execute(stmt, dict(a=address_string)).fetchone()
    ...
    def save_last_api_call_time_to_psql(self, geocoder: str) -> None:
        ...
        with self.connect() as conn:
            stmt = sqlalchemy.text("""
                    ...
                    WHERE geocoder=:b;
                                       """)
            conn.execute(stmt, dict(a=datetime.now(timezone.utc), b=geocoder))
        ...
        with self.connect() as conn:
            stmt = sqlalchemy.text("""
                ...
                WHERE geocoder=:a LIMIT 1;
                                    """)
            last_call = conn.execute(stmt, dict(a=geocoder)).fetchone()
        ...
        with self.connect() as conn:
            stmt = sqlalchemy.text("""
                ...
                WHERE search_id=:a LIMIT 1;
                                   """)
            old_coords = conn.execute(stmt, dict(a=search_id)).fetchone()
            ...
            if current_coords_defined and old_coords is None:
                stmt = sqlalchemy.text("""
                        ...
                        VALUES (:a, :b, :c, :d, CURRENT_TIMESTAMP);
                                           """)
                conn.execute(stmt, dict(a=search_id, b=lat, c=lon, d=coord_type))
                return
            ...
            if current_coords_defined and old_coords is not None:
                ...
                if do_update:
                    stmt = sqlalchemy.text("""
                            ...
                            WHERE search_id=:d;
                                               """)
                    conn.execute(stmt, dict(a=lat, b=lon, c=coord_type, d=search_id))
                    return
            ...
            if (
                ...
                stmt = sqlalchemy.text("""
                        ...
                        WHERE search_id=:b;
                                       """)
                conn.execute(stmt, dict(a=coord_type, b=search_id))
    ...
    def update_search_activities(self, search_num: int, search_activities: list[str]) -> None:
        ...
        sql_text = sqlalchemy.text("""
            ...
            """)
        with self.connect() as conn:
            conn.execute(sql_text, dict(a=search_num))
            ...
            for activity_line in search_activities:
                sql_text = sqlalchemy.text("""
                    ...
                    values ( :a, :b, :c, :d); 
                                        """)
                conn.execute(sql_text, dict(a=search_num, b=activity_line, c='ongoing', d=datetime.now()))
    ...
    def update_search_managers(self, search_num: int, managers: list[str]) -> None:
        ...
        with self.connect() as conn:
            sql_text = sqlalchemy.text("""
                ...
                values ( :a, :b, :c, :d); 
                                    """)
            conn.execute(sql_text, dict(a=search_num, b='managers', c=str(managers), d=datetime.now()))
        ...
        with self.connect() as conn:
            stmt = sqlalchemy.text("""
                DELETE FROM searches WHERE search_forum_num=:a;
                                   """)
            conn.execute(stmt, dict(a=int(search_num)))
