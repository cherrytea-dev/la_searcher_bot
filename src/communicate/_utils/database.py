...
class DBClient:
    ...
    def get_geo_folders_db(self) -> list[tuple]:
        with self.cursor() as cur:
            cur.execute(
                """
    ...
    def get_last_bot_msg(self, user_id: int) -> str:
    ...
    def check_onboarding_step(self, user_id: int, user_is_new: bool) -> tuple[int, str]:
    ...
    def record_search_whiteness(self, user: int, search_id: int, new_mark_value: SearchFollowingMode) -> None:
    ...
    def get_existing_user_settings(self, user_id: int) -> tuple[bool] | None:
        with self.cursor() as cur:
            cur.execute(
                """SELECT
                                    user_id 
    ...
    def get_all_user_preferences(self, user_id: int) -> list[tuple]:
        with self.cursor() as cur:
