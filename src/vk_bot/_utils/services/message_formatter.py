...
def settings_menu_intro() -> str:
    ...
    return (
        'Это раздел с настройками. Здесь вы можете  ввести свои "домашние координаты",'
        ' на основе которых будет рассчитываться расстояние и направление до места поиска.'
        ' Вы в любой момент сможете изменить эти настройки.'
    )
