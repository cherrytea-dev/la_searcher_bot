...
def _fetch_active_searches(user_id: int) -> list[dict]:
    ...
    """
    regions = db().settings.get_user_regions(user_id)
    if not regions:
        return []
...
def _get_user_followed_ids(user_id: int) -> set[int]:
    ...
    try:
        ...
        logger.exception('Error fetching followed searches')
    return followed
...
def handle_view_search_menu(vk_message: VKMessage, state: DialogState | None, user_id: int) -> VKHandlerResult | None:
    ...
    return VKHandlerResult(
        ...
        keyboard=VKKeyboard.search_view_menu(),
    )
...
def handle_active_searches(vk_message: VKMessage, state: DialogState | None, user_id: int) -> VKHandlerResult | None:
    ...
    return VKHandlerResult(
        text=text_result,
        keyboard=VKKeyboard.search_navigation(),
    )
...
def handle_search_follow_menu(vk_message: VKMessage, state: DialogState | None, user_id: int) -> VKHandlerResult | None:
    ...
    text = vk_message.text.strip().lower()
    if text not in ('управление отслеживанием', 'отслеживание поисков'):
        return None

    follow_mode = db().settings.get_search_follow_mode(user_id)
    status = search_follow_mode_on() if follow_mode else search_follow_mode_off()

    return VKHandlerResult(
        text=f'{search_follow_intro()}\n\n{status}',
        keyboard=VKKeyboard.search_follow_menu(),
    )
...
def handle_follow_mode_toggle(vk_message: VKMessage, state: DialogState | None, user_id: int) -> VKHandlerResult | None:
    ...
    if text == 'включить режим отслеживания':
        db().settings.set_search_follow_mode(user_id, True)
        return VKHandlerResult(
            text=search_follow_mode_on(),
            keyboard=VKKeyboard.search_follow_menu(),
        )

    elif text == 'выключить режим отслеживания':
        db().settings.set_search_follow_mode(user_id, False)
        return VKHandlerResult(
            text=search_follow_mode_off(),
            keyboard=VKKeyboard.search_follow_menu(),
        )

    elif text == 'показать отслеживаемые поиски':
...
def handle_follow_unfollow_command(
    ...
    try:
        ...
        logger.exception('Error checking follow state')
    ...
    if action == '+':
        ...
        db().settings.record_search_whiteness(user_id, topic_id, SearchFollowingMode.ON)
        return VKHandlerResult(
        ...
        if current_mode == SearchFollowingMode.OFF:
            ...
            db().settings.record_search_whiteness(user_id, topic_id, '  ')
            return VKHandlerResult(
            ...
            db().settings.record_search_whiteness(user_id, topic_id, SearchFollowingMode.OFF)
            return VKHandlerResult(
