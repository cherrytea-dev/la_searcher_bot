...
class PatternCollection:
    ...
    def _per_by_last_num_patterns(self):
        return [r'.*[^1]1?\d{1,2}(?![0-9])\W{1,5}']

    def _per_fio_patterns(self):
        return [r'.*\W{1,3}[А-Я]\.\W{0,2}[А-Я]\.\W*']

    def _per_human_being_patterns(self):
        return [
            r'(?i).*(женщин[аы]|мужчин[аы]|декушк[аи]|человека?|дочь|сын|жена|муж|отец|мать|папа|мама|'
            r'бабушк[аи]|дедушк[аи])(\W{1,3}|$)'
        ]

    def _per_with_plus_sign_patterns(self):
        return [
            r'(?i)\W{0,3}\+\W{0,2}((женщина|девушка|мама|\d(\W{0,3}человека?\W{1,3})?)|'
            r'(?=[^+]*$)[^+]{0,25}\d{0,3})[^+\w]{1,3}'
        ]

    def _per_age_wo_words_patterns(self):
        return [r'(?i)\d{1,3}(\W{1,4}(?!\d)|$)']

    def _per_age_w_words_patterns(self):
        return [
            r'(?i)(.*\W|^)\d?\d?\d([.,]\d)?\W{0,2}'
    ...
    def _loc_by_individual_patterns(self):
        return [r'(?<![\-–—])*\W{1,3}[\-–—]\W{1,2}(?![\-–—])*']

    def _loc_block_patterns(self):
        return [
            r'(\W[\w-]{3,20}\W)?с\.п\..*',
