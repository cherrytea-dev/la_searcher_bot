...
class PatternCollection:
    ...
    def _per_by_individual_patterns(self) -> list[str]:
        return [
    ...
    def _loc_by_individual_patterns(self) -> list[str]:
...
class PatternCollectionWithMarker:
    def match_type_to_pattern_with_marker(self, pattern_type: PatternType | BlockType) -> list[str]:
        ...
        elif pattern_type == PatternType.PER_AGE_WO_WORDS:
            patterns = self._per_age_wo_words_patterns()

        elif pattern_type == PatternType.PER_WITH_PLUS_SIGN:
            patterns = self._per_with_plus_sign_patterns()

        elif pattern_type == PatternType.PER_HUMAN_BEING:
            patterns = self._per_human_being_patterns()

        elif pattern_type == PatternType.PER_FIO:
            patterns = self._per_fio_patterns()
    ...
    def _loc_block_patterns(self) -> list[str]:
        return [
            r'(\W[\w-]{3,20}\W)?с\.п\..*',
    ...
    def _per_age_w_words_patterns(self) -> list[str]:
        return [
            r'(?i)(.*\W|^)\d?\d?\d([.,]\d)?\W{0,2}'
    ...
    def _per_age_wo_words_patterns(self) -> list[str]:
        return [r'(?i)\d{1,3}(\W{1,4}(?!\d)|$)']

    def _per_with_plus_sign_patterns(self) -> list[str]:
        return [
            r'(?i)\W{0,3}\+\W{0,2}((женщина|девушка|мама|\d(\W{0,3}человека?\W{1,3})?)|'
            r'(?=[^+]*$)[^+]{0,25}\d{0,3})[^+\w]{1,3}'
        ]

    def _per_human_being_patterns(self) -> list[str]:
        return [
            r'(?i).*(женщин[аы]|мужчин[аы]|декушк[аи]|человека?|дочь|сын|жена|муж|отец|мать|папа|мама|'
            r'бабушк[аи]|дедушк[аи])(\W{1,3}|$)'
        ]

    def _per_fio_patterns(self) -> list[str]:
        return [r'.*\W{1,3}[А-Я]\.\W{0,2}[А-Я]\.\W*']

    def _per_by_last_num_patterns(self) -> list[str]:
