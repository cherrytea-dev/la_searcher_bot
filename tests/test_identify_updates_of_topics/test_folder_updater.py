...
from identify_updates_of_topics._utils import folder_updater, forum
...
class TestFolderUpdater:
    def test_parse_one_folder(self, db, mock_http_get):
        ...
        summaries, details = identify_updates_of_topics._utils.folder_updater.FolderUpdater(
            db, forum_search_folder_id
        )._parse_one_folder()
        assert summaries == [
    ...
    def test_process_one_folder(self, db, mock_http_get):
        ...
        with patch.object(forum.ForumClient, 'parse_search_profile', Mock(return_value='foo')):
            update_trigger, changed_ids = folder_updater.FolderUpdater(db, forum_search_folder_id).run()
    ...
    def test_parse_one_comment(self, db, mock_http_get):
        ...
        there_are_inforg_comments = folder_updater.FolderUpdater(db, 1).parse_and_write_one_comment(1, 1)
    ...
    def test_get_cordinates(self, db):
        ...
        with patch('identify_updates_of_topics._utils.folder_updater.rate_limit_for_api'):
            res = folder_updater.FolderUpdater(db, 1).get_coordinates_by_address(data)
        assert res == (None, None)

    def test_update_change_log_and_searches(self, db):
        res = folder_updater.FolderUpdater(db, 1)._update_change_log_and_searches()
        pass
