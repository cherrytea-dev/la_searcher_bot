...
class PatchedSearchUpdater(SearchUpdater):
    forum: 'FakeForum'


@pytest.fixture()
def searches_updater(db_client) -> PatchedSearchUpdater:
    return PatchedSearchUpdater(db_client, FakeForum())
...
class TestFolderUpdaterChangeLogCreation:
    def test_new_search(self, session, searches_updater: PatchedSearchUpdater):
        search_summary = SearchSummaryFactory.build()

        change_log_ids = searches_updater._update_change_log_and_search(search_summary)
    ...
    def test_changed_search(self, session, searches_updater: PatchedSearchUpdater):
        start_replies = 2
        search_summary = SearchSummaryFactory.build(
...
class TestFolderUpdaterChangeDetection:
    def test_no_changes(self, searches_updater: PatchedSearchUpdater):
        snapshot = SearchSummaryFactory.build()
        search = deepcopy(snapshot)

        changes = searches_updater._detect_changes(snapshot, search, False)

        assert not changes

    def test_changed_title(self, searches_updater: PatchedSearchUpdater):
        snapshot = SearchSummaryFactory.build()
        search = deepcopy(snapshot)
    ...
    def test_changed_status(self, searches_updater: PatchedSearchUpdater):
        snapshot = SearchSummaryFactory.build()
        search = deepcopy(snapshot)
    ...
    def test_changed_num_of_replies_no_inforg(self, searches_updater: PatchedSearchUpdater):
        snapshot = SearchSummaryFactory.build()
        search = deepcopy(snapshot)
    ...
    def test_changed_num_of_replies_inforg(self, searches_updater: PatchedSearchUpdater):
        snapshot = SearchSummaryFactory.build()
        search = deepcopy(snapshot)
        search.num_of_replies -= 1

        changes = searches_updater._detect_changes(snapshot, search, True)
