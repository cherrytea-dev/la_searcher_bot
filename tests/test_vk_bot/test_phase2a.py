...
class TestHandleMainMenu:
    ...
    def test_other_menu_button(self, vk_message):
        """ "другие возможности" -> returns None (hidden from main menu)."""
...
class TestHandleSettingsMenu:
    ...
    def test_notification_settings(self, vk_message):
        """ "настроить виды уведомлений" -> returns None (hidden from settings menu)."""
    ...
    def test_age_settings(self, vk_message):
        """ "настроить возрастные группы бвп" -> returns None (hidden from settings menu)."""
    ...
    def test_topic_type_settings(self, vk_message):
        """ "настроить вид поисков" -> returns None (hidden from settings menu)."""
    ...
    def test_forum_linking(self, vk_message):
        """ "связать аккаунты бота и форума" -> returns None (hidden from settings menu)."""
    ...
    def test_vk_linking(self, vk_message):
        """ "связать аккаунты бота и vkontakte" -> returns None (hidden from settings menu)."""
...
class TestHandleCoordinatesAction:
    ...
    def test_view_coordinates(self, vk_message):
        ...
        msg = vk_message(text='посмотреть сохраненные координаты')
        result = handle_coordinates_action(msg, DialogState.not_defined, 12345)
...
class TestHandleFedDistrictSelect:
    ...
    def test_shows_regions(self, vk_message):
        ...
        assert result is not None
        # Flatten all labels from all rows and all columns
        all_labels = [btn['action']['label'] for row in result.keyboard['buttons'] for btn in row]
        assert 'Москва и МО' in all_labels
        assert 'Московская область' in all_labels
    ...
    def test_triggers_pagination_for_many_regions(self, vk_message):
        ...
        assert result is not None
        # Should be inline keyboard with first page (Регион 1-6) + 'ещё →' + 'Завершить'
        assert result.keyboard['inline'] is True
        all_labels = [btn['action']['label'] for row in result.keyboard['buttons'] for btn in row]
        assert 'Регион 1' in all_labels
        assert 'Регион 6' in all_labels
        assert 'Регион 7' not in all_labels  # second page
        assert 'ещё →' in all_labels
        assert 'Завершить' in all_labels

        # Verify callback payloads on inline buttons

        toggle_btn = result.keyboard['buttons'][0][0]
        payload = json.loads(toggle_btn['action']['payload'])
        assert payload['cmd'] == 'paginate_toggle'
        assert payload['region'] == 'Регион 1'

    def test_no_pagination_for_few_regions(self, vk_message):
        ...
        all_labels = [btn['action']['label'] for row in result.keyboard['buttons'] for btn in row]
        assert 'Москва и МО' in all_labels
        assert 'Калужская обл.' in all_labels
        assert 'ещё →' not in all_labels  # no pagination needed
        assert 'Завершить' in all_labels
...
class TestToggleRegionInline:
    ...
    def test_subscribes_to_region(self):
        ...
        result = _toggle_region_inline(12345, 'Москва и МО')

        assert 'добавлен' in result
        self.mock_settings.toggle_region_by_name.assert_called_once()

    def test_unsubscribes_from_region(self):
    ...
    def test_cant_remove_last_region(self):
    ...
    def test_region_not_found(self):
        ...
        result = _toggle_region_inline(12345, 'Неизвестный регион')

        assert 'не найден' in result

    def test_db_error_returns_error_text(self):
